...
for p in tqdm(pp_players, desc="Progress"):
    ...
    if player_name == "OG Anunoby":
        game_logs = playergamelogs.PlayerGameLogs(
            season_nullable="2022-23", player_id_nullable=players.find_players_by_last_name("Anunoby")[0]['id'])
    elif player_name == "Nicolas Claxton":
        game_logs = playergamelogs.PlayerGameLogs(
            season_nullable="2022-23", player_id_nullable=players.find_players_by_full_name("Nic Claxton")[0]['id'])
    else:
        game_logs = playergamelogs.PlayerGameLogs(
            season_nullable="2022-23", player_id_nullable=players.find_players_by_full_name(player_name)[0]['id'])
    ...
    for (i, j) in zip(dpp.NBA_Stat, dpp.Line):
        ...
        last_10_prob = np.round(
            100 * (df.iloc[:10]["OVER_" + i].sum() /
                   df.iloc[:10]["OVER_" + i].size), 1)
        #print(player_name, "OVER", i, j, "LAST_10_G", last_10_prob)
        L10_overs.append(last_10_prob)
        # same for last 2 weeks
        last_2W_prob = np.round(
            100 * (df[df.DATE >= (date.today() - timedelta(14))]["OVER_" + i].sum() /
                   df[df.DATE >= (date.today() - timedelta(14))]["OVER_" + i].size), 1)
        ...
        # same for last 10 games
        last_10_prob = np.round(
            100 * (df.iloc[:10]["UNDER_" + i].sum() /
                   df.iloc[:10]["UNDER_" + i].size), 1)
        #print(player_name, "UNDER", i, j, "LAST_10_G", last_10_prob)
        L10_unders.append(last_10_prob)
        # same for last 2 weeks
        last_2W_prob = np.round(
            100 * (df[df.DATE >= (date.today() - timedelta(14))]["UNDER_" + i].sum() /
                   df[df.DATE >= (date.today() - timedelta(14))]["UNDER_" + i].size), 1)
        #print(player_name, "UNDER", i, j, "LAST_2_WK", last_2W_prob)
        L2W_unders.append(last_2W_prob)

    time.sleep(.600)

df2 = pd.DataFrame({"Name": player_names, "Stat": player_stats, "Line": player_lines,
                   "season_pct_over": season_overs, "L10_pct_over": L10_overs, "L2W_pct_over": L2W_overs,
                    "season_pct_under": season_unders, "L10_pct_under": L10_unders, "L2W_pct_under": L2W_unders})
